...
function drawAll(error, ageCSV, idCSV, occupations) {
  ...
  var colorCircle = d3.scale
    .ordinal()
    .domain([0, 1, 2, 3])
    .range(["#bfbfbf", "#838383", "#4c4c4c", "#1c1c1c"]);

  var colorBar = d3.scale
    .ordinal()
    .domain([
      "16 to 19",
      "20 to 24",
      ...
      if (node.ID in dataById) {
        ...
        if ((node.ID.lastIndexOf(currentID, 0) === 0) & !hidden) {
          ...
          if (drawTitle & showText) {
            ...
            chosenContext.textBaseline = "middle";
            chosenContext.fillText(
              "Total " + commaFormat(node.size) + " (in thousands)",
              nodeX,
              nodeY + -0.75 * nodeR
            );

            //Get the text back in pieces that will fit inside the node
            var titleText = getLines(
              chosenContext,
              node.name,
              nodeR * 2 * 0.7,
              fontSizeTitle,
          ...
          var bars = data[dataById[node.ID]].values,
            ...
            eachBarHeight =
              ((1 - barChartHeightOffset) * 2 * nodeR * barChartHeight) /
              elementsPerBar,
  ...
  function zoomToCanvas(focusNode) {
    ...
    if (focusNode === focus) currentID = "";
      ...
      currentID =
        typeof focusNode.ID === "undefined"
          ? IDbyName[focusNode.name]
          : focusNode.ID.replace(/\.([^\.]*)$/, "");
